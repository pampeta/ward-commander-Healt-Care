...
import { Calendar as CalendarIcon, Plus, Trash2, BookOpen, Clock, AlertCircle, ChevronLeft, ChevronRight, Cloud, ExternalLink, Download } from "lucide-react";
...
function exportarTodosIcs(eventos: EventoAcademico[]) {
  ...
  link.setAttribute('download', `calendario_medico_${new Date().toISOString().split('T')[0]}.ics`);
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          {eventos.length > 0 && (
            <button 
              ...
              className="flex-1 md:flex-none bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-3 rounded-xl md:rounded-lg text-xs md:text-sm font-bold flex justify-center items-center gap-2 transition-colors shrink-0"
              title="Descargar archivo para importar a Google Calendar o Apple Calendar"
            >
              <Download className="w-4 h-4" /> Exportar Todo (.ics)
            </button>
          )}
          <button onClick={() => setModalAbierto(true)} className="flex-1 md:flex-none bg-purple-600 hover:bg-purple-700 text-white px-5 py-3 rounded-xl md:rounded-lg text-sm font-bold flex justify-center items-center gap-2 shadow transition-colors shrink-0"><Plus className="w-5 h-5 md:w-4 md:h-4" /> Agregar Prueba / Evento</button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 transition-all">
          <div className="bg-white sm:rounded-2xl rounded-t-2xl max-w-lg w-full p-5 md:p-6 space-y-4 md:space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 pt-4 border-t border-gray-100">
              <button onClick={() => setModalAbierto(false)} className="order-3 sm:order-1 w-full sm:w-auto px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-bold rounded-xl">Cancelar</button>
              <button 
                onClick={() => {
                  if (!nuevoTitulo.trim() || !nuevaFecha) { alert("Por favor ingresa un título y una fecha."); return; }
                ...
                className="order-2 sm:order-2 w-full sm:w-auto px-4 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 text-sm font-bold rounded-xl flex items-center justify-center gap-1.5 transition-colors"
              >

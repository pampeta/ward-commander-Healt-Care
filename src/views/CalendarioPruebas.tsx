...
export default function CalendarioPruebas() {
  ...
  return (
    // Reducimos paddings globales en celular
    <div className="p-3 md:p-6 max-w-7xl mx-auto space-y-5 md:space-y-6">
      
      {/* CABECERA: Adaptable, flex-col en móvil, flex-row en PC */}
      <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center gap-4 bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-gray-100">
        <div className="space-y-1">
          ...
          <p className="text-xs md:text-sm text-gray-500 max-w-2xl">Visualiza tus fechas importantes en el calendario mensual y revisa los contenidos evaluados.</p>
        </div>
        
        {/* Botón ocupar todo el ancho en móvil, se encoge en PC */}
        <button
          onClick={() => setModalAbierto(true)}
          className="w-full md:w-auto bg-purple-600 hover:bg-purple-700 text-white px-5 py-3 rounded-xl md:rounded-lg text-sm font-bold flex justify-center items-center gap-2 shadow transition-colors shrink-0"
        >
          <Plus className="w-5 h-5 md:w-4 md:h-4" /> Agregar Prueba / Evento
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 md:p-6 space-y-4 overflow-hidden">
        ...
        <div className="flex justify-between items-center border-b border-gray-100 pb-4">
          ...
          <div className="flex items-center gap-1.5 md:gap-2">
            <button 
              ...
              className="px-3 md:px-4 py-1.5 md:py-2 bg-gray-100 hover:bg-gray-200 rounded-lg text-xs md:text-sm font-bold text-gray-700 transition-colors"
            >
              Hoy
            </button>
            <div className="flex bg-gray-100 rounded-lg p-0.5">
              <button onClick={() => cambiarMes(-1)} className="p-1.5 md:p-2 hover:bg-white rounded-md text-gray-700 transition-colors">
        ...
        {/* CONTENEDOR CON SCROLL HORIZONTAL (Prevención para pantallas muy pequeñas) */}
          ...
          <div className="min-w-[280px]">
            ...
            <div className="grid grid-cols-7 gap-1 md:gap-2">
              ...
              {Array.from({ length: diaInicioSemana }).map((_, index) => (
                <div key={`empty-${index}`} className="min-h-[60px] md:min-h-[96px] bg-gray-50/50 rounded-lg md:rounded-xl border border-transparent"></div>
              ))}
              ...
              {Array.from({ length: diasEnMes }).map((_, index) => {
                ...
                return (
                  <div 
                    key={fechaFormateada} 
                    className={`min-h-[60px] md:min-h-[96px] p-1 md:p-2 rounded-lg md:rounded-xl border flex flex-col justify-start md:justify-between overflow-hidden transition-all relative ${
                      esHoy ? 'border-purple-500 bg-purple-50/40 ring-1 md:ring-2 ring-purple-200' : 'border-gray-200 bg-white'
                    }`}
                  >
                    <div className="flex justify-center md:justify-between items-start md:items-center w-full">
                      <span className={`text-[10px] md:text-xs font-bold px-1.5 md:px-2 py-0.5 rounded ${esHoy ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-700'}`}>
                        {numeroDia}
                      </span>
                      {/* Contador de eventos (Oculto en móvil xs para ahorrar espacio, visible en sm y md) */}
                      {eventosDelDia.length > 0 && (
                        <span className="hidden sm:inline-flex text-[9px] md:text-[10px] font-bold bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded-full absolute top-1 right-1 md:relative md:top-0 md:right-0">
                          {eventosDelDia.length}
                        </span>
                      )}
                    </div>

                    {/* Contenedor de eventos visuales */}
                    <div className="mt-1 flex flex-col gap-0.5 w-full">
                      {eventosDelDia.map(ev => {
                         // Asignar colores según el tipo
                         let colorPunto = 'bg-gray-400';
                         let colorFondo = 'bg-gray-100';
                         let colorTexto = 'text-gray-800';
                         ...
                         return (
                           <div key={ev.id} className="w-full flex justify-center md:justify-start" title={`${ev.titulo} (${ev.tipo})`}>
                              {/* EN MÓVIL: Muestra un pequeño punto de color */}
                              <div className={`md:hidden w-2.5 h-2.5 rounded-full ${colorPunto}`}></div>
                              
                              {/* EN PC: Muestra la etiqueta completa con texto truncado */}
      ...
      {/* LISTA DETALLADA DE PRUEBAS Y CONTENIDOS (Apilada) */}
      <div>
          ...
          {eventos.map((ev) => {
            ...
            return (
              <div key={ev.id} className={`bg-white rounded-xl md:rounded-2xl border shadow-sm p-4 md:p-5 flex flex-col justify-between space-y-3 md:space-y-4 relative overflow-hidden ${esHoy ? 'border-2 border-purple-500 bg-purple-50/20' : 'border-gray-100'}`}>
                {esHoy && (
                  <span className="absolute top-0 right-0 bg-purple-600 text-white text-[9px] md:text-[10px] font-bold px-2 md:px-3 py-1 rounded-bl-xl uppercase tracking-wider shadow-sm">
                    ¡Es Hoy! 🎯
                  </span>
                )}
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] md:text-xs font-bold px-2 py-0.5 md:px-2.5 md:py-1 rounded-full border ${
                      ev.tipo === 'Examen Beca / EUNACOM' ? 'bg-red-50 border-red-200 text-red-800' :
                      ev.tipo === 'Prueba / Solemne' ? 'bg-blue-50 border-blue-200 text-blue-800' :
                      ev.tipo === 'Rotación / Turno' ? 'bg-green-50 border-green-200 text-green-800' : 'bg-amber-50 border-amber-200 text-amber-800'
                    }`}>
                      {ev.tipo}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-gray-600 font-mono bg-gray-50 w-fit px-2 py-1 rounded-md border border-gray-100">
                    <Clock className="w-3.5 h-3.5 text-purple-600" /> {ev.fecha.split('-').reverse().join('/')}
                  </div>
                  ...
                  <div className="mt-3 bg-gray-50/80 p-3 rounded-xl border border-gray-100 text-xs space-y-1.5">
                    ...
                    <p className="text-gray-600 leading-relaxed whitespace-pre-wrap pl-5">
                ...
                <div className="pt-3 border-t border-gray-100 flex justify-end">
                  <button
                    ...
                    className="text-gray-400 hover:text-red-600 p-1.5 flex items-center gap-1.5 text-xs font-semibold transition-colors rounded-lg hover:bg-red-50"
                  >
          ...
          {eventos.length === 0 && (
            <div className="col-span-full text-center py-12 md:py-16 text-gray-400 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/50">
              <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-sm font-medium">No hay pruebas o eventos agendados.</p>
                  <p className="text-xs">Usa el botón "Agregar Prueba / Evento" de arriba.</p>
              </div>
            </div>
          )}
        </div>
      ...
      {/* MODAL PARA AGREGAR (Adaptado a pantallas pequeñas) */}
      {modalAbierto && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 transition-all">
          <div className="bg-white sm:rounded-2xl rounded-t-2xl max-w-lg w-full p-5 md:p-6 space-y-4 md:space-y-5 shadow-2xl animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            
            <h2 className="text-base md:text-lg font-extrabold text-gray-900 border-b border-gray-100 pb-3 flex items-center gap-2">
              <AlertCircle className="w-5 h-5 text-purple-600" /> Agendar Nuevo Evento
            </h2>

            <div className="space-y-4">
                <div>
                <label className="block text-[11px] font-bold uppercase text-gray-500 mb-1.5 tracking-wider">Título de la Prueba / Evento <span className="text-red-500">*</span></label>
                <input
                    type="text"
                    placeholder="Ej. Examen final de Pediatría"
                    value={nuevoTitulo}
                    onChange={e => setNuevoTitulo(e.target.value)}
                    className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-purple-200 focus:border-purple-400 transition-all text-gray-900"
                />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label className="block text-[11px] font-bold uppercase text-gray-500 mb-1.5 tracking-wider">Fecha <span className="text-red-500">*</span></label>
                    <input
                    type="date"
                    value={nuevaFecha}
                ...
                <div>
                    <label className="block text-[11px] font-bold uppercase text-gray-500 mb-1.5 tracking-wider">Tipo de Evento</label>
                    <select
                    value={nuevoTipo}
                    onChange={e => setNuevoTipo(e.target.value as any)}
                    className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-purple-200 focus:border-purple-400 transition-all text-gray-900 appearance-none cursor-pointer"
                    >
                ...
                <div>
                <label className="block text-[11px] font-bold uppercase text-gray-500 mb-1.5 tracking-wider">Contenidos que entran</label>
                <textarea
                    rows={4}
                    placeholder="Ej. Temas: Asma infantil, neumonía, crisis obstructiva..."
                    value={nuevosContenidos}
                    onChange={e => setNuevosContenidos(e.target.value)}
                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-purple-200 focus:border-purple-400 transition-all resize-none text-gray-900 leading-relaxed"
                />
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 pt-4 border-t border-gray-100">
              ...
              <button onClick={agregarEvento} className="order-1 sm:order-2 w-full sm:w-auto px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-sm font-bold rounded-xl shadow-md transition-colors">
                Guardar Evento
              </button>
            </div>
          </div>
